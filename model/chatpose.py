...
from .smpl.rotation_converter import rot6d_to_rotmat, batch_axis2matrix
...
class ChatPoseMetaModel:
    ...
    def initialize_chatpose_modules(self, config):
        ...
        # set up type as float
        self.smplx = self.smplx.float()
        # setup renderer
        from .smpl.renderer import set_rasterizer, render_shape
        set_rasterizer(type = 'standard')
        ...
        with torch.no_grad():
            ...
            for i in range(1):
                ...
                if predictions is None:
                    self.model.smplx.float()
                    vertices = self.model.smplx(global_pose=pred_grot.float(), body_pose=pred_body_pose[:,:21].float())[0]

                    if smpl_global_orient is not None:
                        # get gt smpl params
                        smpl_global_orient_curr = smpl_global_orient.clone()
                        gt_global_orient_rotmat = batch_axis2matrix(smpl_global_orient_curr.reshape(-1, 3)).view(batch_size, -1, 3, 3).to(images_clip.device).to(images_clip.dtype)
                        gt_body_pose_rotmat = batch_axis2matrix(smpl_body_pose.reshape(-1, 3)).view(batch_size, -1, 3, 3).to(images_clip.device).to(images_clip.dtype)
                        gt_vertices = self.model.smplx(global_pose=gt_global_orient_rotmat[i:i+1].float(), body_pose=gt_body_pose_rotmat[i:i+1,:21].float())[0]
                    else:
                    ...
                    predictions = shape_image[0] #tensor2image(shape_image[0])

        if kwargs.get('return_smpl', False) and predictions is not None:
